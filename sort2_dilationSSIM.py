...
def dilate_img(img, ksize=3, iterations=1):
    """
    膨張処理を行う関数
    ksize: カーネルサイズ
    iterations: 繰り返し回数
...
def img_ssim(img1, img2):
    ...
    # --- ここで膨張処理を追加 ---
    gray1 = dilate_img(gray1, ksize=3, iterations=1)
    gray2 = dilate_img(gray2, ksize=3, iterations=1)
...
def save_pairs_to_pdf(pairs, imgs1_high, imgs2_high, output_pdf="paired_dilation_ssim.pdf"):
...
def main():
    ...
    print("[4] PDF出力...")
    save_pairs_to_pdf(pairs, imgs1_high, imgs2_high, output_pdf="paired_dilation_ssim.pdf")
