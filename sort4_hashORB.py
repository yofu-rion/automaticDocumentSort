...
def build_score_matrix(imgs1_low, imgs2_low, imgs1_high, imgs2_high, top_k=5):
    ...
    for i, img1_low in enumerate(tqdm(imgs1_low, desc="スコア計算")):
        # --- 低解像度はハッシュで大雑把に比較 ---
        scores = [(j, img_hash_score(img1_low, img2_low, method="phash")) for j, img2_low in enumerate(imgs2_low)]
...
def save_pairs_to_pdf(pairs, imgs1_high, imgs2_high, output_pdf="paired_hash_orb.pdf"):
    ...
    for (p1, p2, score) in tqdm(pairs, desc="PDF作成"):
        ...
        with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp1, \
            ...
            label = f"Pair: img1[{p1}] - img2[{p2 if p2 is not None else 'blank'}], Score={score:.4f}"
...
def main():
    ...
    print("[4] PDF出力...")
    save_pairs_to_pdf(pairs, imgs1_high, imgs2_high, output_pdf="paired_hash_orb.pdf")
