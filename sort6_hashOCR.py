...
# 大雑把: ハッシュ比較
# --------------------
def img_hash_score(img1, img2, method="phash"):
    ...
    if method == "phash":
        ...
        h1, h2 = imagehash.average_hash(pil1), imagehash.average_hash(pil2)
    else:
        h1, h2 = imagehash.dhash(pil1), imagehash.dhash(pil2)

    dist = h1 - h2
    score = 1 - dist / len(h1.hash) ** 2
    return score
...
def extract_text(img, min_font_size=5):
    ...
    for i in range(len(data['text'])):
        ...
        if len(txt) > 1 and h >= min_font_size:  # 小さい文字は無視
            words.append(txt)
    return words

# --------------------
# 細かい比較: TF-IDF + コサイン類似度
# --------------------
...
# --------------------
# スコアマトリクス構築（OCRはキャッシュ利用）
# --------------------
def build_score_matrix(imgs1_low, imgs2_low, words1_cache, words2_cache, top_k=5):
    matrix = np.zeros((len(imgs1_low), len(imgs2_low)))
    for i, img1_low in enumerate(tqdm(imgs1_low, desc="スコア計算")):
        # 低解像度でハッシュ比較
        scores = [(j, img_hash_score(img1_low, img2_low)) for j, img2_low in enumerate(imgs2_low)]
        scores.sort(key=lambda x: x[1], reverse=True)
        top_candidates = scores[:top_k]

        words1 = words1_cache[i]
        for j, _ in top_candidates:
            words2 = words2_cache[j]
            score = tfidf_cosine_similarity(words1, words2)
            matrix[i, j] = score
    return matrix
...
# PDF出力
# --------------------
def save_pairs_to_pdf(pairs, imgs1_high, imgs2_high, output_pdf="paired_hash_ocr.pdf"):
...
def main():
    ...
    print("[2] スコアマトリクス構築...")
    matrix = build_score_matrix(imgs1_low, imgs2_low, words1_cache, words2_cache, top_k=5)

    print("[3] ペアリング...")
    pairs = pair_unique(matrix, threshold=0.1)

    print("[4] PDF出力...")
    save_pairs_to_pdf(pairs, imgs1_high, imgs2_high, output_pdf="paired_hash_ocr.pdf")
